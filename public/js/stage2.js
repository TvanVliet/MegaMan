...
Stage2.prototype = {
    create: function () {
        ...
        //images/sprites/text
        background3 = this.background3 = this.game.add.sprite(0, 0, 'background3');
        bmpText = this.add.bitmapText(200, 20, 'alien', 'Stage 2', 32);
    ...
    update: function () {
        ...
        if (this.game.time.now > firingTimer) {
            ...
            if (livingEnemies.length > 0) {
                ...
                eShootSound.play();
                firingTimer = this.game.time.now + 1500;
            }
        }
    },
    ...
    createEnemy: function () {
        for (var y = 0; y < 2; y++) {
            for (var x = 0; x < 3; x++) {
                enemy = enemies.create(this.game.world.width + 50, Math.random() * 400, 'spinner', this.game.rnd.integerInRange(0, this.game.world.width + 10));
                enemy.anchor.setTo(0.5, 0.5);
                enemy.scale.setTo(1.5);
                enemy.animations.add('flie', [0, 1, 2, 3], 20, true);
                enemy.play('flie');
            }
        }
    },
